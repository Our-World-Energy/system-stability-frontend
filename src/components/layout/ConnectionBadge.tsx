...
const config: Record<ConnectionState, { label: string; dot: string; text: string; pulse: boolean }> = {
  ...
  failed: { label: 'Reconnecting…', dot: 'bg-critical-bright', text: 'text-critical-bright', pulse: true },
}
...
export function ConnectionBadge({ connection }: { connection: ConnectionState }) {
  ...
  return (
    <span
      className="flex h-9 shrink-0 items-center gap-2 rounded-lg border border-line px-3 font-mono text-xs"
